...
export const FAMILY_INDEX_ERROR_CODES = Object.freeze({
  ...
  LOOKUP_FETCH_FAILED: 'LOOKUP_FETCH_FAILED',
...
function normalizeBaseUrl(value) {
  const base = String(value || DEFAULT_BASE_URL);
...
function aliasShard(alias) {
  const first = alias[0];
  return first && /[a-z0-9]/.test(first) ? first : '_other';
}

function abortError(cause) {
  return new FamilyIndexError(FAMILY_INDEX_ERROR_CODES.ABORTED, 'Family index request was aborted.', { cause });
}
...
function isAbortError(error) {
  return error?.name === 'AbortError' || error?.code === FAMILY_INDEX_ERROR_CODES.ABORTED;
}
...
async function fetchJson(fetchImpl, url, { signal, code, language, shard } = {}) {
  ...
  try {
    ...
    throw new FamilyIndexError(code, `Family index fetch failed: ${url}`, { language, shard, cause: error });
  }
  if (!response?.ok) throw new FamilyIndexError(code, `Family index fetch failed: ${url}`, { language, shard, cause: response });
  try {
    return await response.json();
  } catch (error) {
    throw new FamilyIndexError(code, `Family index JSON parse failed: ${url}`, { language, shard, cause: error });
  }
}
...
function lookupShardMeta(manifest, shard) {
  return manifest.lookup_shards.find(item => item?.shard === shard) || null;
}
...
function familyMatch(entry, familyMeta, familyId, lookupAlias, queryRoot) {
  ...
  const anchors = [entry.family_surface_anchor, ...(Array.isArray(familyMeta?.aliases) ? familyMeta.aliases : [])]
    .map(normalizeAlias)
    .filter(Boolean)
    .filter(anchor => searchForm.includes(anchor))
  ...
  return {
    type: 'family',
    family_id: familyId,
    family_label: familyMeta?.label || familyId,
    family_confidence: familyMeta?.confidence || null,
...
export function createFamilyIndexLoader(options = {}) {
  ...
  async function cachedJson(path, meta = {}) {
    ...
    diagnostics.loadedShards.push(path);
    return value;
  }
  ...
  async function resolveFamilies(root, { signal } = {}) {
    ...
    for (let length = normalized.length; length >= MIN_ALIAS_LENGTH; length -= 1) {
      ...
      const families = lookup?.[alias];
      if (Array.isArray(families) && families.length) {
        diagnostics.resolvedAlias = alias;
        diagnostics.resolvedFamilyIds = families.map(item => item.id);
  ...
  async function loadCandidateEntries(language, root, { signal } = {}) {
    ...
    const resolved = await resolveFamilies(root, { signal });
    if (!resolved.families.length) {
      diagnostics.familyCandidateIds = 0;
      return [];
    }
    ...
    for (const familyRef of resolved.families) {
      ...
      for (const source of bucket.members) {
        ...
        const entry = {
          ...
          family_ids: [...new Set([...(Array.isArray(source.family_ids) ? source.family_ids : []), familyRef.id])],
          family_label: bucket.meta?.label || familyRef.label || familyRef.id,
          family_confidence: bucket.meta?.confidence || familyRef.confidence || null,
          family_aliases: Array.isArray(bucket.meta?.aliases) ? bucket.meta.aliases : [],
          match: familyMatch(source, bucket.meta, familyRef.id, resolved.alias, root)
        };
    ...
    diagnostics.familyCandidateIds = entries.length;
    return entries;
  }

  function getFamilyIndexDiagnostics() {
  ...
  function clearCache() {
    cache.clear();
    manifestCache = null;
    diagnostics.manifestLoaded = false;
    diagnostics.loadedShards = [];
    diagnostics.resolvedFamilyIds = [];
    diagnostics.resolvedAlias = null;
    diagnostics.familyCandidateIds = 0;
  }

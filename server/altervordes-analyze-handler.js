...
import { normalizeInterfaceLanguage } from '../api/lib/interface-language.js';
import {
  buildAltervordesSystemPrompt,
  buildAltervordesUserPrompt
} from '../api/lib/altervordes-prompts.js';
import { sanitizeGeneratedDerivativeClaims } from './altervordes-noun-guard.js';

const DERIVATION_CONTEXT = JSON.parse(
  readFileSync(new URL('../api/interal-derivation-context.json', import.meta.url), 'utf8')
);
...
async function runAnalysis(payload, interfaceLanguage) {
  ...
  const normalized = normalizeResult(extractJson(result.content), input, result.model);
  return { ok: true, analysis: sanitizeGeneratedDerivativeClaims(normalized, input) };
}

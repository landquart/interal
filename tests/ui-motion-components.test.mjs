...
assert.match(sidebar, /id="sidebar-outline"/, 'sidebar source exposes its unchanged outer contour');
assert.match(sidebar, /id="sidebar-divider"[\s\S]*M118 8 H156[\s\S]*V269[\s\S]*C133 14\.716 126\.284 8 118 8/, 'sidebar source preserves all four rounded divider joins');
assert.match(core, /sidebar_corrected_v2\.svg[\s\S]*#sidebar-outline[\s\S]*#sidebar-divider/, 'menu icon references both parts from the supplied SVG');
assert.match(css, /\.sidebar-state-divider\{[^}]*transition:transform 240ms cubic-bezier\(\.2,0,0,1\)/, 'divider movement uses the requested timing and easing');
assert.match(css, /body\.menu-open \.sidebar-state-divider\{transform:translateX\(269px\)\}/, 'divider reaches the symmetric right-hand position');
assert.match(core, /MutationObserver[\s\S]*aria-expanded[\s\S]*menu-open/, 'icon accessibility follows the actual sidebar state');

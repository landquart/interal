...
assert.match(script, /makeAssociativeLemmaMetadata\(\{[\s\S]*elementType: state\.elementType,[\s\S]*translationWord: state\.translationWord,[\s\S]*targetMeaning: state\.targetMeaning/, 'generated cards delegate element type and lemma metadata to the shared builder');
assert.doesNotMatch(script, /partOfSpeech\s*[,):=]|part_of_speech/, 'generated associative cards never include a part of speech');

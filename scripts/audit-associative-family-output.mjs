...
const materializedFamilies=new Set();const memberBuckets=Object.fromEntries(manifest.languages.map(x=>[x,Array(256).fill(0)]));let materializedMemberships=0;
for(const language of manifest.languages){const dir=join(root,'members',language);const files=(await readdir(dir)).filter(x=>x.endsWith('.json')).sort();assert(files.length===256,`${language} member shards ${files.length}`);for(const file of files){const b=basename(file,'.json');const shard=await readJson(join(dir,file));for(const [id,values] of Object.entries(shard)){assert(bucket(id)===b,`${language} member bucket ${id}`);assert(familyIds.has(id),`${language} member family ${id}`);assert(values.length,`${language} empty family ${id}`);const seen=new Set();for(const value of values){assert(lemmas[language].has(value.lemma_id),`${language} unknown lemma ${value.lemma_id}`);assert(!seen.has(value.lemma_id),`${language} duplicate member ${id}/${value.lemma_id}`);assert(value.components?.length,`${language} member components ${id}`);for(const c of value.components)assert(c.canonical_candidate&&c.evidence?.length,`${language} member evidence ${id}`);seen.add(value.lemma_id);materializedMemberships++;memberBuckets[language][parseInt(b,16)]++;}materializedFamilies.add(id);}}}
...
for(const id of ['family:pede','family:ocul','family:manu']){const f=(await readJson(join(root,'families',`${bucket(id)}.json`)))[id];const m=(await readJson(join(root,'members','en',`${bucket(id)}.json`)))[id];assert(f.support===m.length&&f.language_support?.en===m.length,`seed support mismatch ${id}: ${f.support}/${m.length}`);}

#!/usr/bin/env node
// Apply four locked language decisions to the Latin sonus family.
...
const ledgerBytes = await readFile('audit/associative-family-v5/sonus-language-decisions.json');
const ledger = JSON.parse(ledgerBytes);
const repairName = 'prune_sonus_false_memberships';
...
assert(ledger.source_run_id === 35647932153 && ledger.family_id === 'ety:1e7afcab5044', 'unexpected source/family');
assert(JSON.stringify(ledger.decisions.map(item => item.language)) === JSON.stringify(['en', 'fr', 'es', 'it']), 'unexpected languages');
assert(provenance.source_run_id === ledger.source_run_id, 'source mismatch');
...
assert(family?.etymon_keys?.includes('la:sonus') && family.support === 6670, 'family changed');
...
const retainedCount = ledger.decisions.reduce((sum, item) => sum + item.keep.length, 0);
assert(family.support === 9 && retainedCount === 9 && Object.values(family.language_support).reduce((sum, count) => sum + count, 0) === 9, 'family support mismatch');
const largest = [];
const suspicious = [];
...
report.repository_materialization.sonus_memberships_removed = removed;
report.repository_materialization.sonus_ledger_sha256 = ledgerSha;
provenance.repository_repairs.push({ repair: repairName, family_id: ledger.family_id, removed_memberships: removed, retained_memberships: retainedCount, decision_ledger_sha256: ledgerSha, source_run_id: ledger.source_run_id, not_individual_lexical_annotation: true });

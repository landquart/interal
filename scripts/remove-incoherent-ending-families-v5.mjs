#!/usr/bin/env node
// Apply six explicit ending-based family decisions; lock members and evidence before writing.
...
const ledgerBytes = await readFile('audit/associative-family-v5/surface-ending-decisions.json');
const ledger = JSON.parse(ledgerBytes);
const repairName = 'remove_incoherent_ending_surface_families';
...
assert(ledger.source_run_id === 35647932153 && ledger.decisions.length === 6, 'unexpected ledger/source');
...
report.review_required_families -= ledger.decisions.length;
report.aliases_in_lookup -= deletedAliases;
const removedIds = new Set(ledger.decisions.map(item => item.family_id));
const largest = [];
const suspicious = [];
const keepTop = (list, family, compare) => { list.push(family); list.sort(compare); if (list.length > 50) list.length = 50; };
for (let index = 0; index < 256; index += 1) {
  const path = join(root, 'families', `${index.toString(16).padStart(2, '0')}.json.gz`);
  for (const family of Object.values(familyShards.get(path) || await readJson(path))) {
...
report.repository_materialization.incoherent_ending_memberships_removed = removedMembers;
report.repository_materialization.ending_family_ledger_sha256 = ledgerSha;

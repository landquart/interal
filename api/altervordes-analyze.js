...
async function callYandex(messages, withResponseFormat = true) {
  ...
  return {
    content: getAiText(responseJson, text),
    model: payload.model
  };
}
...
async function runAnalysis(payload, interfaceLanguage) {
  ...
  const messages = [
    {
      role: 'system',
      content: buildAltervordesSystemPrompt(input.interfaceLanguage, DERIVATION_CONTEXT, options)
    },
    {
      role: 'user',
      content: buildAltervordesUserPrompt(input, options)
    }
  ];
  ...
  return {
    ok: true,
    analysis: normalizeResult(extractJson(result.content), input, result.model)
  };
}

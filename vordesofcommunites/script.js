...
function bindJsonModal() {
  ...
  byId('checkBtn')?.addEventListener('click', async () => { readState(); if(!validateForm()){ alert(t('validationRequired')); render(); return; } const ok=await autoCheckWithQwen(); if(ok) state.checked = true; render(); updateCheckedVisibility(); updateResetButtonVisibility(); window.InteralFormDraft?.save?.(); });
  byId('app')?.addEventListener('input', () => { updateResetButtonVisibility(); if (state.checked && !window.InteralFormDraft?.isRestoring?.()) { state.checked = false; readState(); render(); return; } if (state.checked) { readState(); renderResult(); if(state.qwenError) byId('resultBox').insertAdjacentHTML('beforeend', `<p class="muted">${escapeHtml(state.qwenError)}</p>`); updateCheckedVisibility(); } });

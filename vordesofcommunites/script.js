...
function bindJsonModal() {
  ...
  byId('checkBtn')?.addEventListener('click', async () => { readState(); if(!validateForm()){ alert(t('validationRequired')); render(); return; } const ok=await autoCheckWithQwen(); if(ok) state.checked = true; render(); updateCheckedVisibility(); updateResetButtonVisibility(); });

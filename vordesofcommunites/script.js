...
async function resetState(){ if (!(await (window.InteralUI?.confirmReset?.() ?? Promise.resolve(window.confirm(t('reset')))))) return; window.InteralUI?.clearCurrentPageState?.(); document.dispatchEvent(new CustomEvent('interal:page-reset')); state = { word: '', part_of_speech: 'adverb', domain: '', translations: { en:'', de:'', fr:'', es:'', it:'', ru:'' }, criteria: [false, false, false], answers: ['', '', ''] }; closeJsonModal(); render(); }

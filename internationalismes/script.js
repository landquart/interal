...
async function createCardOnServer(card) { const title = card?.interal?.word || card?.title || 'Untitled card'; const response = await fetch(CARDS_API_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, category: card?.vord_type || 'internationalism', payload: card }) }); const data = await response.json().catch(() => null); if (!response.ok || !data?.ok) throw new Error(data?.error || `HTTP ${response.status}`); return data.card?.payload || { ...card, id: data.id, discussionId: data.id }; }
...
function currentJsonText() { return byId('jsonCardOutput')?.value || ''; }
async function openJsonModal() { readState(); const output = byId('jsonCardOutput'); if (output) output.value = JSON.stringify(await makeCard(), null, 2); const modal = byId('jsonCardModal'); modal?.classList.add('show'); modal?.setAttribute('aria-hidden', 'false'); }

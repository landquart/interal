...
async function resetAll() { if (!(await (window.InteralUI?.confirmReset?.({ message: t('resetConfirm') }) ?? Promise.resolve(window.confirm(t('resetConfirm')))))) return; window.InteralUI?.clearCurrentPageState?.(); document.dispatchEvent(new CustomEvent('interal:page-reset')); state = { word: '', part_of_speech: 'noun', evidence: {}, autoPassed: {}, manualOverride: {}, matchMeta: {}, isSearching: false, searchError: '' }; render(); }

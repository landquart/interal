...
function bindJsonModal() {
  ...
  byId('app')?.addEventListener('input', () => { if(state.checked && !window.InteralFormDraft?.isRestoring?.()){ state.checked=false; readState(); state.manuallyEdited=true; state.finalized=false; render(); return; } if(state.checked){ readState(); state.manuallyEdited=true; state.finalized=true; renderResult(); } updateResetButtonVisibility();  });
  byId('app')?.addEventListener('change', () => { if(state.checked && !window.InteralFormDraft?.isRestoring?.()){ state.checked=false; readState(); state.manuallyEdited=true; state.finalized=false; render(); return; } if(state.checked){ readState(); state.manuallyEdited=true; state.finalized=true; renderResult(); } updateResetButtonVisibility();  });
}

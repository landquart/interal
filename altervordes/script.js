...
function applyI18n(){ const u=t(); $('pageTitle').textContent=u.pageTitle; $('paramsTitle').textContent=u.paramsTitle; $('translationLabel').textContent=u.translationLabel; $('candidateLabel').textContent=u.candidateLabel; $('posLabel').textContent=u.partOfSpeechLabel; $('commentLabel').textContent=u.commentLabel; $('checkBtn').textContent=u.check; $('jsonBtn').textContent=u.createJson; $('decisionTitle').textContent=u.resultTitle; $('jsonCardTitle').textContent=u.jsonTitle; $('useAuthorBlockLabel').textContent=u.useAuthor; $('authorDisplayNameLabel').textContent=u.authorName; $('authorContactTypeLabel').textContent=u.contactType; $('authorContactValueLabel').textContent=u.contact; $('generateJsonCardBtn').textContent=u.generateJson; $('jsonCardOutputLabel').textContent=u.outputJson; $('closeJsonCardBtn').setAttribute('aria-label',u.closeJson); $('copyJsonCardBtn').setAttribute('aria-label',u.copyJson); $('copyJsonCardBtn').setAttribute('title',u.copyJson); $('downloadJsonCardBtn').setAttribute('aria-label',u.downloadJson); $('downloadJsonCardBtn').setAttribute('title',u.downloadJson); $('resetBtn').setAttribute('aria-label', lang()==='ru'?'Сбросить':'Reset'); $('resetBtn').setAttribute('title', lang()==='ru'?'Сбросить':'Reset'); const current=$('posInput').value||'noun'; $('posInput').innerHTML=POS_VALUES.map(v=>`<option value="${v}">${escapeHtml(u.pos[v])}</option>`).join(''); $('posInput').value=POS_VALUES.includes(current)?current:'noun'; if(lastAnalysis) renderAnalysis(lastAnalysis); }
...
async function createJsonCard(){ if(!canCreateCard(lastAnalysis)) throw Error(t().cardBlocked); const state=collectState(); const now=new Date().toISOString(); const base={ id:CARD_PREFIX+uuid12(), section:CARD_SECTION, type:'alter-vordes', algorithmStep:6, title:lastAnalysis.recommendedForm||state.candidate, selectedForm:lastAnalysis.recommendedForm||state.candidate, translation:lastAnalysis.inputTranslation||state.translation, partOfSpeech:lastAnalysis.partOfSpeech||state.partOfSpeech, status:'candidate', translations:lastAnalysis.translations||{controlLanguages:{},auxiliaryLanguages:{}}, analysis:lastAnalysis.analysis||{}, derivation:normalizeDerivation(lastAnalysis.derivation||{}), risks:Array.isArray(lastAnalysis.risks)?lastAnalysis.risks:[], shortConclusion:lastAnalysis.shortConclusion||'', model:{name:MODEL_NAME,role:'advisory evaluator',finalDecisionByHuman:true}, createdAt:now, updatedAt:now };
  if ($('useAuthorBlock')?.checked) { const name=$('authorDisplayName').value.trim(); const type=$('authorContactType').value; const contact=$('authorContactValue').value.trim(); if (name || contact) base.author={name, contactType:type, contact}; }
...
function fill(s={}){ $('translationInput').value=s.translation||''; $('candidateInput').value=s.candidate||''; $('posInput').value=POS_VALUES.includes(s.partOfSpeech)?s.partOfSpeech:'noun'; $('commentInput').value=s.comment||''; if(s.lastAnalysis){ renderAnalysis(s.lastAnalysis); setJsonEnabled(canCreateCard(s.lastAnalysis)); } updateResetVisibility(); }
...
document.addEventListener('DOMContentLoaded',()=>{ applyI18n(); setJsonEnabled(false); updateResetVisibility(); $('checkBtn').onclick=()=>evaluateAlterWord().catch(e=>showNotice(e.message||t().apiError,'error')); $('jsonBtn').onclick=()=>{ if(!canCreateCard(lastAnalysis)){ showNotice(t().cardBlocked,'error'); return; } openJsonModal(); }; $('generateJsonCardBtn').onclick=()=>createJsonCard().catch(e=>showNotice(e.message||t().apiError,'error')); $('closeJsonCardBtn').onclick=closeJsonModal; $('jsonCardModal').onclick=(event)=>{ if(event.target===$('jsonCardModal')) closeJsonModal(); }; $('useAuthorBlock').onchange=(event)=>{ $('jsonAuthorFields').style.display=event.target.checked?'grid':'none'; }; $('copyJsonCardBtn').onclick=()=>{ const text=$('jsonCardOutput').value; if(!text) return; navigator.clipboard.writeText(text); $('copyJsonCardBtn').classList.add('is-copied'); $('copyJsonCardBtn').setAttribute('title',t().copiedJson); setTimeout(()=>{ $('copyJsonCardBtn').classList.remove('is-copied'); $('copyJsonCardBtn').setAttribute('title',t().copyJson); },1500); }; $('downloadJsonCardBtn').onclick=()=>{ const text=$('jsonCardOutput').value; if(!text) return; const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type:'application/json'})); a.download=`${lastCard?.id||'altervordes'}.json`; a.click(); URL.revokeObjectURL(a.href); }; $('resetBtn').onclick=()=>resetState().catch(e=>showNotice(e.message||t().apiError,'error')); ['translationInput','candidateInput','commentInput'].forEach(id=>$(id).addEventListener('input',updateResetVisibility)); $('posInput').addEventListener('change',updateResetVisibility); document.addEventListener('keydown',event=>{ if(event.key==='Escape') closeJsonModal(); }); document.addEventListener('interal:languagechange',applyI18n); restoreStateFromUrl(); });

...
document.addEventListener('DOMContentLoaded',()=>{ document.documentElement.lang = currentLang(); applyI18n(); setJsonEnabled(false); updateResetVisibility(); $('checkBtn').onclick=()=>evaluateAlterWord().catch(e=>showNotice(e.message||t().apiError,'error')); try { if (!window.InteralJsonCardModal) throw new Error('InteralJsonCardModal is not loaded.'); if (!window.InteralJsonCards) throw new Error('InteralJsonCards is not loaded.'); window.InteralJsonCardModal.init({ openButtonId:'jsonBtn', getLanguage: lang, getTexts: () => ({ close:t().closeJson, title:t().jsonTitle, useAuthor:t().useAuthor, authorName:t().authorName, contactType:t().contactType, contact:t().contact, rememberAuthor:t().rememberAuthor, clearSavedAuthor:t().clearSavedAuthor, generate:t().generateJson, generating: lang()==='en'?'Generating...':'Генерация...', output:t().outputJson, copy:t().copyJson, copied:t().copiedJson, copiedTitle:lang()==='en'?'Copied':'Скопировано', download:t().downloadJson, empty:lang()==='en'?'Generate the JSON card first.':'Сначала сгенерируйте JSON-карточку.', unavailable:lang()==='en'?'The JSON card is available only after a successful check.':'JSON-карточка доступна только после успешной проверки.' }), buildCard: async ({author, onProgress}={})=>{ if(!hasSuccessfulCheck || !canCreateCard(lastAnalysis)) throw Error(t().cardBlocked); onProgress?.(lang()==='en'?'Building card...':'Сборка карточки...'); return createJsonCard(author, { onProgress }); }, formatCard:(card)=>JSON.stringify(card,null,2), getFilename:()=> 'altervord-card.json' }); } catch (error) { console.error('Could not initialize JSON card module:', error); showNotice(lang()==='en'?'Could not load the JSON card module. Reload the page.':'Не удалось загрузить модуль JSON-карточек. Обновите страницу.','error'); } $('resetBtn').onclick=()=>resetState().catch(e=>showNotice(e.message||t().apiError,'error')); ['translationInput','candidateInput','commentInput'].forEach(id=>$(id).addEventListener('input',()=>{ invalidateAnalysis(); updateResetVisibility(); })); $('posInput').addEventListener('change',()=>{ invalidateAnalysis(); updateResetVisibility(); }); document.addEventListener('interal:languagechange',()=>{ document.documentElement.lang = currentLang(); applyI18n(); }); restoreStateFromUrl(); });

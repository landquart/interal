...
function applyI18n(){ const u=t(); $('pageTitle').textContent=u.pageTitle; $('paramsTitle').textContent=u.paramsTitle; $('translationLabel').textContent=u.translationLabel; $('candidateLabel').textContent=u.candidateLabel; $('posLabel').textContent=u.partOfSpeechLabel; $('commentLabel').textContent=u.commentLabel; setButtonStatus('#checkBtn', u.check, false); $('jsonBtn').textContent=u.createJson; $('decisionTitle').textContent=u.resultTitle; $('jsonCardTitle').textContent=u.jsonTitle; $('useAuthorBlockLabel').textContent=u.useAuthor; $('authorDisplayNameLabel').textContent=u.authorName; $('authorContactTypeLabel').textContent=u.contactType; $('authorContactValueLabel').textContent=u.contact; setButtonStatus('#generateJsonCardBtn', u.generateJson, false); $('jsonCardOutputLabel').textContent=u.outputJson; $('closeJsonCardBtn').setAttribute('aria-label',u.closeJson); $('copyJsonCardBtn').setAttribute('aria-label',u.copyJson); $('copyJsonCardBtn').setAttribute('title',u.copyJson); $('downloadJsonCardBtn').setAttribute('aria-label',u.downloadJson); $('downloadJsonCardBtn').setAttribute('title',u.downloadJson); $('resetBtn').setAttribute('aria-label', lang()==='ru'?'Сбросить':'Reset'); $('resetBtn').setAttribute('title', lang()==='ru'?'Сбросить':'Reset'); const current=$('posInput').value||'noun'; $('posInput').innerHTML=POS_VALUES.map(v=>`<option value="${v}">${escapeHtml(u.pos[v])}</option>`).join(''); $('posInput').value=POS_VALUES.includes(current)?current:'noun'; if(lastAnalysis) renderAnalysis(lastAnalysis); }

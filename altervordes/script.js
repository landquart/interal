...
const SHORT_CONCLUSION_LANGUAGES = ['en','de','fr','es','it','ru'];
...
function localizedShortConclusion(value, preferredLanguage = lang()) {
  if (typeof value === 'string') return value.trim();
  if (!value || typeof value !== 'object' || Array.isArray(value)) return '';
  const preferred = typeof value[preferredLanguage] === 'string' ? value[preferredLanguage].trim() : '';
  if (preferred) return preferred;
  for (const code of SHORT_CONCLUSION_LANGUAGES) {
    const fallback = typeof value[code] === 'string' ? value[code].trim() : '';
...
async function evaluateAlterWord(){ const state=collectState(); validateState(state); hasSuccessfulCheck = false; setButtonStatus('#checkBtn', t().checking, true, { loading: true }); showNotice(t().loading,'info'); setJsonEnabled(false); try { const { lastAnalysis: _omit, ...payload } = state; setButtonStatus('#checkBtn', t().evaluating, true, { loading: true }); const res=await fetch(`${API_BASE}/api/altervordes-analyze`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payload,interfaceLanguage:lang()})}); setButtonStatus('#checkBtn', t().processingAnalysis, true, { loading: true }); const data=await res.json().catch(()=>null); if(!res.ok || !data?.ok) throw Error(data?.error || t().apiError); lastAnalysis=data.analysis || data.result; hasSuccessfulCheck = canCreateCard(lastAnalysis); renderAnalysis(lastAnalysis); setJsonEnabled(canCreateCard(lastAnalysis)); window.InteralFormDraft?.save?.(); setTimeout(() => window.InteralFormDraft?.save?.(), 120); hideNotice(); updateResetVisibility(); setButtonStatus('#checkBtn', t().done, true, { loading: true }); } catch (error) { setButtonStatus('#checkBtn', t().error, false, { loading: false }); throw error; } finally { setButtonStatus('#checkBtn', t().check, false, { loading: false }); } }
...
function renderAnalysis(r){ lastAnalysis=r; $('resultSection').hidden=false; const u=t(); const [cls,msg]=statusText(r); const a=r?.analysis||{}; const risks=Array.isArray(r?.risks)&&r.risks.length?r.risks:[u.noRisks]; const form=r?.recommendedForm||r?.selectedForm||$('candidateInput').value.trim()||'—'; const conclusion=localizedShortConclusion(r?.shortConclusion)||'—'; $('resultBox').innerHTML=`<div class="decision-card"><div class="status ${cls}">${escapeHtml(msg)}</div><div class="decision-main"><div class="decision-main-item"><span class="decision-label">${u.recommendedForm}</span><strong class="decision-form">${escapeHtml(form)}</strong></div><div class="decision-main-item"><span class="decision-label">${u.conclusion}</span><p>${escapeHtml(conclusion)}</p></div></div><details open><summary>${u.controlLanguages}</summary>${langRows(CONTROL_LANGUAGES,r?.translations?.controlLanguages)}</details><details open><summary>${u.auxiliaryLanguages}</summary>${langRows(AUXILIARY_LANGUAGES,r?.translations?.auxiliaryLanguages)}</details><details open><summary>${u.analysis}</summary><ul class="analysis-list">${Object.keys(u.labels).map(k=>`<li><b>${escapeHtml(u.labels[k])}:</b> ${escapeHtml(a[k]||'—')}</li>`).join('')}</ul></details><details><summary>${u.risks}</summary><ul class="risk-list">${risks.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></details></div>`; }
